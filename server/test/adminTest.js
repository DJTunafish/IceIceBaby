...
describe("admin test", function() {
  it("should return string of lollollolol", function(done) {
    server
    .get("/test")
    .expect("Content-type", /json/)
    .end(function(err, res) {
      res.status.should.equal(200);
      res.body.text.should.equal('lollolollolol');
      done();
    });
  });
  ...
  /*it("Create a course as admin", function(done) {
    server
    .post("/createcourse")
    .send({gencode: 'qwe', coursecode: 'DAT123', name:'prog av databaser',
       description: 'en kurs där man lär sig', admin:'adamin'})
    .expect("Content-type",/json/)
    .expect(200)
    .end(function(err, res) {
      res.status.should.equal(200);
      console.log(res.body);
      //res.body.gencode.should.equal('aaaaab');
      done();
    });
  });*/
  ...
  it("Not able to reg a course with a gencode already in use", function(done) {
    server
    .post("/createcourse")
    .send({gencode: 'abcdf', coursecode: 'DAT123', name:'prog av databaser',
       description: 'en kurs där man lär sig', admin:'adamin'})
    .expect("Content-type",/json/)
    .expect(500)
    .end(function(err, res) {
